...
THICKNESS = 2
### yolov5 weight 
model_dir = "E:\datnd\datnd\DATN_workersite\weights\\best.pt"
...
while ret:
    start = time.time()
    id_deepsort = [1, 2, 3, 13, 4,5,6,7,8]
    preds = model(frame)
    tracked_id_kpts = []
    for ith, pred in enumerate(preds.xyxyn[0]):
        x1, y1, x2, y2 = patch_extractor(pred)
            
        tracked_id_kpts.append((id_deepsort[ith], visualisation(frame, x1, y1, x2, y2), (x1, y1-20)))        # (id_deepsort, keypoints, coordinate(x1, y1))
        
        cv2.rectangle(frame, (x1, y1), (x2, y2), colors_dict[pred[-1].item()], 2)


    if cnt % 5 == 0:
        id_kpts = process_frame(frame, tracked_id_kpts, id_kpts, recognizer)
